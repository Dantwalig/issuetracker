...
export class MyWorkService {
  ...
  async getDashboard(userId: string) {
    ...
    // Stats summary - Calculate in-memory to avoid extra DB queries
    const totalAssigned = assignedIssues.length;
    const totalInProgress = assignedIssues.filter(i => i.status === 'IN_PROGRESS').length;
    const totalTodo = assignedIssues.filter(i => i.status === 'TODO').length;
